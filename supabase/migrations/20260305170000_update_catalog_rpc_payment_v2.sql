-- Atualiza catalog_create_pedido para suportar campos de pagamento e parcelas.
...
begin
  ...
  where confeitaria_id = v_confeitaria_id
    and (telefone = v_cliente_telefone or telefone = regexp_replace(v_cliente_telefone, '\D', '', 'g'))
  limit 1;
  ...
  insert into public.pedidos (
    ...
    nullif(payload->>'forma_pagamento_id', '')::uuid,
    nullif(payload->>'forma_pagamento_nome', ''),
    coalesce(nullif(payload->>'parcelas', '')::int, 1),
